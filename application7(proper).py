...
def compare():
    ...
    for chunk in pythoninputbychunksbylinesbykeywords:
        ...
        for line in chunk:
            ...
            # (this effectively links these statements to four-syllable words; obviously I will have to make changes about this sort of detail)
                ...
                pythonbinarybychunksbylinesbykeywords[chunkindex][lineindex] = [1, 0] # line was previously: 0; now it is: [0, 1, 0, 1]; now [1, 0, 1]; no [1, 0]
    ...
    for line in scannedinputbylinesbywords:
        ...
        for word in line:
            ...
            syllables = [x for x in syllables if "." not in x]
            syllables = [x for x in syllables if "," not in x]
            print(f"syllables post-period-remove: {syllables}")
            q = soup.find_all("b")
            print(f"q: {q}")
            if len(syllables) > 1:
                ...
                for i in range(len(q)):
                    #p = q[i]
                    print(f"Boldsyllable found: {q[i]}")
                    ...
                    print(f"indexofboldsyll: {indexofboldsyllable}")
            ...
            # the latter.
            # elif "<b>" and "</b>" in word:
            #     scannedbinarybylinesbywords[lineindex][wordindex] = 1
            wordaslist = list(word)
            #print(f"WORDASLIST: {wordaslist}")
            if (len(q) == 1) and wordaslist[:3] == ['<', 'b', '>'] and (wordaslist[(len(wordaslist)-1) - 4:] == ['<', '/', 'b', '>', '?'] or wordaslist[(len(wordaslist)-1) - 3:] == ['<', '/', 'b', '>']):
                wordlistasstring = ''.join(wordaslist)
                #print(f"One-syllable, emphasized word found: {wordlistasstring}")
                scannedbinarybylinesbywords[lineindex][wordindex] = 1

    ######
    print(f"verse as binary: {scannedbinarybylinesbywords}")
        ...
        if pythonchunk in scannedbinarybylinesbywords:
            ...
            for pythonline in pythonchunk:
                ...
                #     outputlistbylines.append(pythonlineasverseword)
                    ...
                    # if pythonline == scannedbinarybylinesbywords[verselineindex][i]:
                    #     pythonlineasverseword = scannedinputbylinesbywords[verselineindex][i]
                    #     outputlistbylines.append(pythonlineasverseword)
                        #pythonbinary_line_index_in_versebinary_line = scannedbinarybylinesbywords[verselineindex].index(pythonline)
                        # print(f"pythonline as binary: {pythonline}")
                        # print(f"verselinebinary: {scannedbinarybylinesbywords[verselineindex]}")
                        # print(f"location in versebinary: {pythonbinary_line_index_in_versebinary_line}")
                        # print(f"verselineinput of verselinebinary: {scannedinputbylinesbywords[verselineindex][pythonbinary_line_index_in_versebinary_line]}")
                        ...
                        #     scannedbinarybylinesbywords[verselineindex][pythonbinary_line_index_in_versebinary_line] = None

                    #Don't use this. No matter how tempting it is. You WILL GET MORE VARIETY VIA THE ABOVE METHOD ONLY (which is guarenteed to change at least
                    # chunk-by-chunk)
                    #elif (scannedbinarybylinesbywords[verselineindex].count(1) == 1) or (scannedbinarybylinesbywords[verselineindex].count(0) == 1):


                # else: ###### This clause takes everything that wasn't found in the max... in some sense it should therefore be separate;
                #       ###### after ALL python chunks have been evaluated with the above method. But then there would be only partial finishes for
                #       ###### those chunks... —————— NO; that is why  we have the count > 1 clause; to maintain each chunk to its end.
                #     for verseline in scannedbinarybylinesbywords:
                #         verselineindex = scannedbinarybylinesbywords.index(verseline)
                #         if pythonline in verseline:
                #             versewordindex = scannedbinarybylinesbywords[verselineindex].index(pythonline)
                #             pythonlineasverseword = scannedinputbylinesbywords[verselineindex][versewordindex]
                #             outputlistbylines.append(pythonlineasverseword)
                #             if (scannedbinarybylinesbywords[verselineindex].count(1) > 1) and (scannedbinarybylinesbywords[verselineindex].count(0) > 1):
                #                 scannedbinarybylinesbywords[verselineindex][versewordindex] = None
                #             #print(f"pythonlineasverseword (singelword!): {pythonlineasverseword}")

    print(f"Outputlist, pre-stringify of syllables: {outputlistbylines}")

    outputlistbylines = [[str(x) if not isinstance(x, str) else x for x in y] if isinstance(y, list) else y for y in outputlistbylines]
    print(f"Outputlist, post-stringify of syllables: {outputlistbylines}")
    outputlistbylines = ["".join(word) if isinstance(word, list) else word for word in outputlistbylines]
    print(f"Outputlist, post join-syllables: {outputlistbylines}")

    outputasplaintext = " ".join(outputlistbylines)

    print(outputasplaintext)

    return render_template("compare.html", file1=file1, file2=outputasplaintext)
