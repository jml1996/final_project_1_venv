...
def compare():
    ...
    for pythonchunk in pythonbinarybychunksbylinesbykeywords:
        if pythonchunk in scannedbinarybylinesbywords:
            ...
            del pythonchunk ###### added 12/9/17 at 2:48 PM

    ###### separated: (it was starting with a new chunk every time)
    ...
    for pythonchunk in pythonbinarybychunksbylinesbykeywords:
        ...
        best_verselineindex = scannedbinarybylinesbywords.index(verseline_binary_as_list_to_be_searched)

        #for pythonline in pythonbinarybychunksbylinesbykeywords[pythonchunkindex]:
        for pythonline in pythonbinarybychunksbylinesbykeywords[pythonchunkindex]:
            if pythonline in scannedbinarybylinesbywords[best_verselineindex]:
                pythonbinary_line_index_in_versebinary_line = scannedbinarybylinesbywords[best_verselineindex].index(pythonline)
                print(f"pythonline as binary: {pythonline}")
                print(f"verselinebinary: {scannedbinarybylinesbywords[best_verselineindex]}")
                print(f"location in versebinary: {pythonbinary_line_index_in_versebinary_line}")
                print(f"verselineinput of verselinebinary: {scannedinputbylinesbywords[best_verselineindex][pythonbinary_line_index_in_versebinary_line]}")
                # pop it so that there are no duplicates; we don't need this value in....
                # nope; can't pop it; that would change indices...
                pythonlineasverseword = scannedinputbylinesbywords[best_verselineindex][pythonbinary_line_index_in_versebinary_line]
                if (scannedbinarybylinesbywords[best_verselineindex].count(1) > 1) and (scannedbinarybylinesbywords[best_verselineindex].count(0) > 1):
                    scannedbinarybylinesbywords[best_verselineindex][pythonbinary_line_index_in_versebinary_line] = None
                outputlistbylines.append(pythonlineasverseword)
            else:
                # shuffledbinarywords = scannedbinarybylinesbywords
                # random.shuffle(shuffledbinarywords)
                #for verseline in shuffledbinarywords:
                for verseline in scannedbinarybylinesbywords:
